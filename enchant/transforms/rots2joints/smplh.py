...
class SMPLH(Rots2Joints):
    ...
    def forward(self,
        ...
        if nohands:
            from enchant.tools.easyconvert import to_matrix
            # still axis angle
...
def smplh_to(jointstype, data, trans):
    from enchant.utils.joints import get_root_idx

    if "mmm" in jointstype:
        from enchant.utils.joints import smplh2mmm_indexes
        indexes = smplh2mmm_indexes
        data = data[..., indexes, :]

        # make it compatible with mmm
        if jointstype == "mmm":
            from enchant.utils.joints import smplh_to_mmm_scaling_factor
        ...
        from enchant.utils.joints import smplh2smplnh_indexes
