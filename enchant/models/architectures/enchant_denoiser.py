...
from enchant.models.architectures.tools.embeddings import (TimestepEmbedding,
                                                       Timesteps)
from enchant.models.operator import PositionalEncoding
from enchant.models.operator.cross_attention import (SkipTransformerEncoder,
                                                 TransformerDecoder,
                                                 TransformerDecoderLayer,
                                                 TransformerEncoder,
                                                 TransformerEncoderLayer)
from enchant.models.operator.position_encoding import build_position_encoding
from enchant.utils.temos_utils import lengths_to_mask


class EnchantDenoiser(nn.Module):
    ...
    def __init__(self,
        ...
        self.music_encoded_dim = music_encoded_dim
        ...
        if self.condition in ["music", "music_uncond"]:
            # music condition
            # project time from music_encoded_dim to latent_dim
            self.time_proj = Timesteps(music_encoded_dim, flip_sin_to_cos,
                                       freq_shift)
            self.time_embedding = TimestepEmbedding(music_encoded_dim,
                                                    self.latent_dim)
            # project time+music to latent_dim
            if music_encoded_dim != self.latent_dim:
                # todo 10.24 debug why relu
                self.emb_proj = nn.Sequential(
                    nn.ReLU(), nn.Linear(music_encoded_dim, self.latent_dim))
    ...
    def forward(self,
        ...
        if self.condition in ["music", "music_uncond"]:
            # music_emb [seq_len, batch_size, music_encoded_dim] <= [batch_size, seq_len, music_encoded_dim]
            encoder_hidden_states = encoder_hidden_states.permute(1, 0, 2)
            music_emb = encoder_hidden_states  # [num_words, bs, latent_dim]
            # musicembedding projection
            if self.music_encoded_dim != self.latent_dim:
                # [1 or 2, bs, latent_dim] <= [1 or 2, bs, music_encoded_dim]
                music_emb_latent = self.emb_proj(music_emb)
            else:
                music_emb_latent = music_emb
            if self.abl_plus:
                emb_latent = time_emb + music_emb_latent
            else:
                emb_latent = torch.cat((time_emb, music_emb_latent), 0)
        elif self.condition in ['action']:
